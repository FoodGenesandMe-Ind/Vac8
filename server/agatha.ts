...
export async function runAgathaTurn(
  ...
  while (result.toolCalls.length > 0 && round < maxRounds) {
    round++;
    const toolResults: { toolUseId: string; name: string; result: string }[] = [];
    ...
    if (activeProvider === "anthropic") {
      result = await anthropic.continueWithToolResultsAnthropic(
        system,
        history,
        result.text,
        toolResults,
      ...
      result = await openai.continueWithToolResultsOpenAI(
        system,
        history,
        result.text,
        result.toolCalls,
