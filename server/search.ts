...
export function buildGoogleFlightsUrl(opts: {
  ...
  if (opts.origin) params.set("tfs", `from ${opts.origin}`);
...
export const webSearchProvider: PriceProvider = {
  name: "webSearch",
...
export function parsePriceFromSnapshot(snapshot: string): number | undefined {
  const prices = [...snapshot.matchAll(/\$[\d,]+(?:\.\d{2})?/g)].map((m) =>
    parseFloat(m[0].replace(/[$,]/g, ""))
  );
