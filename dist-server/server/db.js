...
export function listVacations() {
    const rows = db.prepare("SELECT data FROM vacations ORDER BY updated_at DESC").all();
    return rows.map((r) => VacationPlanSchema.parse(JSON.parse(r.data)));
}
export function getVacation(id) {
    const row = db.prepare("SELECT data FROM vacations WHERE id = ?").get(id);
    if (!row)
        return null;
    return VacationPlanSchema.parse(JSON.parse(row.data));
}
export function saveVacation(plan) {
    const now = new Date().toISOString();
    const updated = { ...plan, updatedAt: now };
    const data = JSON.stringify(updated);
    db.prepare(`INSERT INTO vacations (id, data, created_at, updated_at) VALUES (?, ?, ?, ?)
     ON CONFLICT(id) DO UPDATE SET data = excluded.data, updated_at = excluded.updated_at`).run(updated.id, data, updated.createdAt, now);
...
export function listMessages(vacationId) {
    const rows = db
...
export function addMessage(msg) {
    db.prepare("INSERT INTO messages (id, vacation_id, role, content, created_at) VALUES (?, ?, ?, ?, ?)").run(msg.id, msg.vacationId, msg.role, msg.content, msg.createdAt);
}
