...
export async function runTool(name, input, ctx) {
    ...
    switch (name) {
        ...
        case "promote_suggestion": {
            ...
            if (!sug)
                return { result: "Suggestion not found." };
            const { promoted: _, ...item } = sug;
            plan = saveVacation(recalcTotals({
                ...plan,
                workingPlan: [...plan.workingPlan, item],
                suggestions: plan.suggestions.map((s) => s.id === sid ? { ...s, promoted: true } : s),
            }));
